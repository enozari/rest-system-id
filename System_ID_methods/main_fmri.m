function main_fmri(section, resolution, varargin)
...
switch section
    case 'submit'
        ...
        if run_on_cluster
            if numel(varargin) >= 1
                max_jobs = varargin{1};                                     % The maximum number of jobs that should be submitted to the cluster at once.                                          % If a second optional parameter is provided in calling main(1, max_jobs), it indicates the maximum amount of jobs that should be submitted to the cluster at once. 
            else
                max_jobs = inf;
            end
            ...
            if ~exist(['main_data_dnn_' resolution], 'dir')                 % Directory to keep track of which jobs have run to the point of running the DNN method. This is important since the DNN trainer occationally hangs indefinitely, and the job needs to be killed and restarted (done automatically by the code below).
                mkdir(['main_data_dnn_' resolution])
            end 
            ...
            for i_subj = 1:n_subj
                ...
                for i_cv = i_cv_vals(i_subj, :)
                    ...
                    if ~exist(filename, 'file') ... &&
                        ...
                        [~, cmdout] = system(['qsub ./main.sh ' resolution ' ' subj ' ' num2str(i_cv) ' ' num2str(parcel)]);
            ...
            for i_subj = 1:n_subj
                subj = subjs{i_subj};
                for i_cv = i_cv_vals(i_subj, :)
                    main_fmri('run', resolution, subj, i_cv, parcel)
                end
            end
        end
    ...
    case 'run'
        ...
        [model_summary, R2, R2_pw, runtime, whiteness, whiteness_pw, model, Y_hat] = ...
            all_methods(Y, Y_pw, TR, test_range, MMSE_memory, [subj '_' num2str(i_cv)]); % Calling the routine that runs all system id methods.
        ...
        save(filename, 'model_summary','R2', 'R2_pw', 'runtime', 'whiteness', 'whiteness_pw', ...
            'Y_hat', 'model', '-v7.3');                               % Saving the data to be used in the subsequent main(2) call.
