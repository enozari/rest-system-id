function [model, R2, whiteness_p, Y_hat] = nonlinear_DNN(Y, n_AR_lags, hidden_width, hidden_depth, exe_env, ...
    InitialLearnRate_drop_factor, test_range)
%NONLINEAR_BOLD Fitting and cross-validating a nonlinear model using ReLU
...
if nargin < 6 || isempty(InitialLearnRate_drop_factor)
    InitialLearnRate_drop_factor = 0.1;
end
...
for i = 1:hidden_depth
    layers = [layers, fullyConnectedLayer(hidden_width), reluLayer];
end
...
while 1
    ...
    if any(arrayfun(@(i)any(isnan(net.Layers(i).Weights(:))), 2:2:2*(hidden_depth+1)))
        InitialLearnRate = InitialLearnRate * 0.1;
    else
        break
    end
end
InitialLearnRate = InitialLearnRate * InitialLearnRate_drop_factor;         % Additional layer of conservatism, which is necessary due to the random initialization of trainNetwork.
options = trainingOptions(fixedTrainingOptions{:}, ...
    'MaxEpochs', 100, ...
    'InitialLearnRate', InitialLearnRate);
net = trainNetwork(permute(Y_train_lags, [3 4 1 2]), Y_train_diff', layers, options);
...
Y_test_diff_hat = predict(net, permute(Y_test_lags, [3 4 1 2]), 'ExecutionEnvironment', exe_env)'; % DNN's prediction of the derivative (approximated by first difference) of Y_test
